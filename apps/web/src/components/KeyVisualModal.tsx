...
import { keyVisualCompositionToBattleRenderState } from "@/app/key-visual-battle-adapter"
import { buildKeyVisualComposition } from "@/app/key-visual-composition"
...
export function KeyVisualModal({ variant, onClose, displayOptions }: KeyVisualModalProps) {
  ...
  const composition = useMemo(
    () => buildKeyVisualComposition(elapsedMs, variant),
    [elapsedMs, variant],
  )
  const renderState = useMemo(
    () => keyVisualCompositionToBattleRenderState(composition),
    [composition],
  )
  ...
  const handleDownloadPng = useCallback(() => {
    ...
    /* 1. 黒背景 */
    ctx.fillStyle = tokenRgba("void", 1)
    ...
    /* 3. スキャンライン */
    ctx.fillStyle = tokenRgba("signal", 0.03)
       ...
       ゲーム座標: player=(240, playerY), barrier≈30px, 弾は r≈33 の位置 */
    {
      ...
      // 軌道アーク
      ctx.globalAlpha = 0.3
      ctx.strokeStyle = tokenRgba("memory", 0.76)
      ...
      // コア
      ctx.globalAlpha = 0.5
      ctx.fillStyle = tokenRgba("memory", 0.82)
      ctx.beginPath()
    ...
    vig.addColorStop(0, tokenRgba("void", 0))
    vig.addColorStop(1, tokenRgba("void", 0.5))
    ctx.fillStyle = vig
    ctx.fillRect(0, 0, W, H)
    ...
    grad.addColorStop(0, tokenRgba("void", 0))
    grad.addColorStop(0.25, tokenRgba("void", 0.08))
    grad.addColorStop(0.67, tokenRgba("void", 0.35))
    grad.addColorStop(1, tokenRgba("void", 0.82))
    ctx.fillStyle = grad
    ...
    if (!displayOptions.reduceFlashing) {
      // 7b. グリッチバンドは key visual 書き出し専用です。低点滅設定では静的ロゴだけを出力します。
      ...
      ctx.clip()
      ctx.globalAlpha = 0.8
      ctx.fillStyle = tokenRgba("danger", 0.85)
      ctx.shadowColor = tokenRgba("danger", 0.6)
      ...
      ctx.clip()
      ctx.globalAlpha = 0.8
      ctx.fillStyle = tokenRgba("signalBright", 0.85)
      ctx.shadowColor = tokenRgba("signalBright", 0.6)
      ctx.shadowBlur = isWindowed ? 4 : 8
      ctx.setTransform(1, 0, Math.tan((-glitchSkew * Math.PI) / 180), 1, -glitchShift, 0)
      ctx.fillText("MAGNOLIA", W / 2, logoY)
      ctx.restore()
    ...
    lineGrad.addColorStop(0.12, tokenRgba("signal", 0.18))
    lineGrad.addColorStop(0.5, tokenRgba("signalBright", 0.6))
    lineGrad.addColorStop(0.88, tokenRgba("signal", 0.18))
    ...
    ctx.shadowColor = tokenRgba("signal", 0.35)
    ctx.shadowBlur = isWindowed ? 4 : 8
    ctx.fillStyle = tokenRgba("textSecondary", 0.55)
  ...
  }, [displayOptions.reduceFlashing, variant])
    ...
    <div className="key-visual-modal-backdrop" onClick={onClose} role="presentation">
      ...
      >
        ...
        <div className="key-visual-modal__overlay">
          <MagnoliaLogo
            as="div"
            className="key-visual-modal__logo"
