...
export function ExploreCanvas({
  ...
  useEffect(() => {
    ...
    prevPlayerX = renderState.playerPosition.x
    prevPlayerY = renderState.playerPosition.y
  }, [
    renderState,
    snapshot,
    presentation,
...
function resolveCameraViewport(input: {
...
function readRebootViewportHeight(baseViewportHeight: number, progress: number): number {
...
function drawExploreScene(
  ...
  ctx.globalAlpha = 1
  updateTrail(input.renderState.playerPosition.x, input.renderState.playerPosition.y, input.timeMs)
  if (trailOpacity > 0.001) {
    ctx.save()
    ctx.globalAlpha = trailOpacity
    drawTrail(ctx, input.timeMs, input.viewport, input.width, input.height, input.padding)
    ctx.restore()
  }
...
function readReleaseViewportHeight(baseViewportHeight: number, progress: number): number {
  if (progress < 0.24) {
    const t = easeInOutSine(progress / 0.24)
...
function updateTrail(wx: number, wy: number, now: number) {
  ...
  const cutoff = trailStoppedAt > 0
    ? trailStoppedAt - TRAIL_DISSOLVE_MS
    : now - TRAIL_MAX_MS
  let dropCount = 0
  while (dropCount < trailNodes.length && trailNodes[dropCount].time < cutoff) {
    dropCount++
  }
  if (dropCount > 0) trailNodes.splice(0, dropCount)
...
function trailOpacity(nodeIndex: number, nodeCount: number, now: number): number {
  ...
  if (trailStoppedAt > 0) {
    const d = Math.min(1, (now - trailStoppedAt) / TRAIL_DISSOLVE_MS)
...
function drawTrail(
  ...
  for (let i = 0; i < N; i++) {
    const p = worldToCanvas(viewport, W, H, pad, trailNodes[i].wx, trailNodes[i].wy)
    cx[i] = p.x
    cy[i] = p.y
  }
  ...
  const dissolveProg = trailStoppedAt > 0
    ? Math.min(1, (now - trailStoppedAt) / TRAIL_DISSOLVE_MS)
    : 0
...
function drawLightMotes(
  ...
  ctx.save()
  for (let i = lightMotes.length - 1; i >= 0; i--) {
    const m = lightMotes[i]
    const age = now - m.born
    if (age > m.life) {
      lightMotes.splice(i, 1)
      continue
    }
    m.wx += m.vx
    m.wy += m.vy
