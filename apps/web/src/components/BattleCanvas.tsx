...
export function BattleCanvas({ renderState, transparentBg, shipVariant }: BattleCanvasProps) {
  ...
  useEffect(() => {
    ...
    if (!transparentBg) {
      drawBattleBackground(context, renderState.elapsedMs)
    } else {
      // KV モード: SignalBackdropCanvas の上に干渉帯・波紋を重ねて深みを出す
      drawTransparentAtmosphere(context, renderState.elapsedMs)
    }
    ...
    for (const hazard of renderState.hazards) {
      drawHazard(context, hazard, renderState.elapsedMs)
    }

    // --- support fields ---
    for (const field of renderState.supportFields) {
      drawSupportField(context, field, renderState.elapsedMs)
    }

    // --- enemy projectiles ---
    for (const p of renderState.projectiles) {
      if (p.side === "enemy") {
        drawEnemyProjectile(context, p, renderState.elapsedMs, renderState)
      }
    }

    // --- pickups ---
    for (const pickup of renderState.pickups) {
      drawBattlePickup(context, pickup, renderState.elapsedMs)
    }

    // --- enemies ---
    for (const enemy of renderState.enemies) {
      drawEnemy(context, enemy, renderState.elapsedMs, renderState)
    }

    // --- player projectiles ---
    for (const p of renderState.projectiles) {
      if (p.side === "player") {
        drawPlayerProjectile(context, p, renderState.elapsedMs, renderState)
      }
    }

    // --- barrier gauge ---
    drawBarrierGauge(context, renderState)

    // --- player ship ---
    drawPlayerShip(context, renderState, shipVariant)
