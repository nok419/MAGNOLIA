...
import {
  ...
  lerpScalar,
  seededUnit,
} from "@/render/explore/explore-render-utils"
...
export function drawRebootBackdrop(
  ...
  base.addColorStop(0, hex("voidBase"))
  base.addColorStop(0.58, hex("voidBase"))
  base.addColorStop(1, hex("voidBase"))
  ctx.fillStyle = base
  ...
  radial.addColorStop(0, rgba("signalPrimary", radialAlpha))
  radial.addColorStop(0.4, rgba("voidDepth", (radialAlpha * 0.45)))
  radial.addColorStop(1, rgba("voidBase", 0))
  ctx.fillStyle = radial
  ctx.fillRect(0, 0, width, height)

  ctx.save()
  ctx.strokeStyle = rgba("signalPrimary", ((0.024 + progress * 0.018) * awake))
  ctx.lineWidth = 1
  const ringCount = 4
...
function drawRebootAmbient(
  ...
  grad.addColorStop(0, rgba("lineStrong", (alpha * 1.4)))
  grad.addColorStop(0.35, rgba("signalPrimary", alpha))
...
function drawRebootCoreSeed(
  ...
  halo.addColorStop(0, rgba("signalReadable", (0.28 * eased * breath)))
  halo.addColorStop(0.34, rgba("signalPrimary", (0.18 * eased)))
  halo.addColorStop(1, rgba("voidBase", 0))
  ctx.fillStyle = halo
  ctx.fillRect(center.x - 100, center.y - 100, 200, 200)

  ctx.fillStyle = rgba("signalReadable", (0.72 * eased))
  ctx.shadowColor = rgba("lineStrong", 0.9)
  ctx.shadowBlur = 18
  ctx.beginPath()
  ...
  ctx.shadowBlur = 12
  ctx.strokeStyle = rgba("lineStrong", (0.36 * eased))
  ctx.lineWidth = 1
...
function drawRebootAssemblyAperture(
  ...
  ctx.globalCompositeOperation = "lighter"
  ctx.strokeStyle = rgba("lineStrong", 0.36)
  ctx.lineWidth = 1
  ...
  ctx.shadowBlur = 0
  ctx.strokeStyle = rgba("signalReadable", (0.18 * alpha))
  ctx.lineWidth = 0.8
...
function drawRebootFinalHold(
  ...
  grad.addColorStop(0, rgba("signalReadable", (alpha * 0.8)))
  grad.addColorStop(0.42, rgba("signalPrimary", alpha))
  grad.addColorStop(1, rgba("voidBase", 0))
  ctx.fillStyle = grad
...
function drawRebootCompletionHalo(
  ...
  ctx.strokeStyle = rgba("signalReadable", (0.34 + reveal * 0.2))
  ctx.lineWidth = 1.2
  ctx.shadowColor = rgba("signalReadable", (0.24 + reveal * 0.18))
  ctx.shadowBlur = 14
  ctx.beginPath()
  ...
  grad.addColorStop(0, rgba("signalReadable", (0.14 * input.progress * pulse)))
  grad.addColorStop(0.42, rgba("signalPrimary", (0.12 * input.progress)))
  grad.addColorStop(1, rgba("voidBase", 0))
  ctx.fillStyle = grad
  ctx.fillRect(0, 0, input.width, input.height)

  ctx.strokeStyle = rgba("signalReadable", (0.34 * input.progress))
  ctx.lineWidth = 1.6
  ctx.shadowColor = rgba("signalReadable", (0.24 * input.progress))
  ctx.shadowBlur = 18
  ctx.beginPath()
  ctx.arc(input.center.x, input.center.y, radius, 0, TAU)
  ctx.stroke()
  ctx.strokeStyle = rgba("signalReadable", (0.18 * reveal))
  ctx.lineWidth = 1
  ctx.shadowBlur = 10
  ctx.beginPath()
...
function drawRebootPhasePulse(
  ...
  ctx.save()
  ctx.strokeStyle = rgba("lineStrong", alpha)
  ctx.lineWidth = 1 + (1 - localProgress) * 1.2
  ctx.shadowColor = rgba("lineStrong", 0.6)
  ctx.shadowBlur = 16
  ctx.beginPath()
  ctx.arc(center.x, center.y, radius, 0, TAU)
  ctx.stroke()
...
function drawRebootIgnition(
  ...
  for (const ring of rings) {
    ...
    ctx.shadowColor = rgba("lineStrong", 0.8)
    ctx.shadowBlur = 24
    ctx.beginPath()
  ...
  if (coreAlpha > 0.01) {
    ...
    coreGrad.addColorStop(0, rgba("signalReadable", coreAlpha))
    coreGrad.addColorStop(0.3, rgba("lineStrong", (coreAlpha * 0.6)))
    coreGrad.addColorStop(1, rgba("signalPrimary", 0))
...
function drawRebootField(
  ...
  for (let r = 0; r < ringCount; r++) {
    ...
    ctx.strokeStyle = rgba("lineStrong", ringAlpha)
    ctx.lineWidth = 0.9
  ...
  for (let index = 0; index < lineCount; index += 1) {
    ...
    lineGradient.addColorStop(0, rgba("signalPrimary", 0))
    lineGradient.addColorStop(0.58, rgba("signalPrimary", 0.18))
    lineGradient.addColorStop(1, rgba("signalReadable", 0.55))
  ...
  for (let s = 0; s < streamCount; s++) {
    ...
    streamGrad.addColorStop(0, rgba("signalPrimary", 0))
    streamGrad.addColorStop(1, rgba("signalReadable", streamAlpha))
    ctx.strokeStyle = streamGrad
    ctx.lineWidth = 1.6
    ctx.lineCap = "round"
    ctx.shadowColor = rgba("lineStrong", 0.5)
    ctx.shadowBlur = 6
    ctx.beginPath()
    ctx.moveTo(sx, sy)
    ctx.lineTo(ex, ey)
  ...
  for (let moteIndex = 0; moteIndex < 40; moteIndex += 1) {
    ...
    ctx.fillStyle = moteIndex % 5 === 0 ? hex("signalReadable") : hex("lineStrong")
    ctx.beginPath()
...
function drawRebootConstruction(
  ...
  ctx.strokeStyle = rgba("signalReadable", shellAlpha)
  ctx.lineWidth = 1.2 / K // ctx.scale で太線にならないよう逆補正
  ctx.shadowColor = rgba("lineStrong", 0.85)
  ctx.shadowBlur = 22 / K
  ...
  ctx.fillStyle = rgba("lineStrong", 1)
  ...
  if (resolvedProgress > 0.001) {
    ...
    drawShip(ctx, {
      ...
      center: { x: 0, y: 0 },
      scale: 0.72,
      stroke: rgba("signalReadable", 0.9),
      fill: rgba("lineStrong", 0.08),
      lineWidth: 0.9 / K,
      glow: { color: rgba("lineStrong", 0.8), blur: 10 / K },
      core: {
        color: rgba("signalReadable", 0.9),
        glowColor: rgba("lineStrong", 0.95),
        glowBlur: 8 / K,
        radius: 1.8 / K,
  ...
  scanGrad.addColorStop(0, rgba("lineStrong", 0))
  scanGrad.addColorStop(0.5, rgba("signalReadable", (0.6 * input.progress)))
  scanGrad.addColorStop(1, rgba("lineStrong", 0))
  ctx.globalAlpha = 1
  ...
  // ── 3. 外周リング 2 重 ──
  ctx.strokeStyle = rgba("lineStrong", (0.18 + input.progress * 0.22))
  ctx.lineWidth = 1.2 / K
  ctx.beginPath()
  ...
  ctx.strokeStyle = rgba("lineStrong", (0.35 + input.progress * 0.28))
  ctx.lineWidth = 1 / K
  ...
  for (let index = 0; index < 6; index += 1) {
    ...
    ctx.fillStyle = index % 2 === 0 ? hex("signalReadable") : hex("lineStrong")
    ctx.beginPath()
...
function drawRebootConstructionRings(
  ...
  for (let ringIndex = 0; ringIndex < 4; ringIndex += 1) {
    ...
    ctx.shadowColor = rgba("signalPrimary", 0.55)
...
function drawRebootHullFragments(
  ...
  for (let index = 0; index < 34; index += 1) {
    ...
    ctx.shadowColor = rgba("lineStrong", 0.75)
...
function drawRebootInternalCircuit(
  ...
  ctx.strokeStyle = rgba("lineStrong", 0.72)
  ctx.lineWidth = 0.55 / scale
  ctx.shadowColor = rgba("signalPrimary", 0.5)
