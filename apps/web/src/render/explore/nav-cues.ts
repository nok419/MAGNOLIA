...
export type NavCueKind = "equipment" | "item" | "mission" | "warp"
...
const NAV_CUE_COLORS: Record<NavCueKind, { role: VisualRgbRole }> = {
  equipment: { role: "line" },
  item:      { role: "text" },
  mission:   { role: "memory" },
  warp:      { role: "memory" },
...
function drawNavCuePinpoint(
  ...
  shadow.addColorStop(0, tokenRgba(color.role, 0.09 * (0.85 + 0.15 * twinkle01)))
  shadow.addColorStop(0.55, tokenRgba(color.role, 0.03))
  shadow.addColorStop(1, tokenRgba(color.role, 0))
  ctx.fillStyle = shadow
  ctx.beginPath()
  ctx.arc(target.point.x, target.point.y, shadowR, 0, TAU)
  ctx.fill()
  ...
  halo.addColorStop(0, tokenRgba(color.role, 0.3 * brightness))
  halo.addColorStop(0.45, tokenRgba(color.role, 0.1 * brightness))
  halo.addColorStop(1, tokenRgba(color.role, 0))
  ctx.fillStyle = halo
  ctx.beginPath()
  ctx.arc(target.point.x, target.point.y, haloR, 0, TAU)
  ctx.fill()

  // Layer 3: 中央コア。shadowBlur でふわっとした芯光。
  ctx.shadowColor = tokenRgba(color.role, 0.55)
  ctx.shadowBlur = 6
  ctx.fillStyle = tokenRgba(color.role, 0.65 + 0.25 * twinkle01)
  ctx.beginPath()
  ctx.arc(target.point.x, target.point.y, (1.4 + 0.35 * twinkle01) * scale, 0, TAU)
  ctx.fill()
  ctx.shadowBlur = 0

  // Layer 4: 地味なパーティクル。3 粒、それぞれ独立した位相で浮かび上がっては消える。
  // 周回せず、星から少し離れた位置にごく弱い光点として明滅させる。
  drawNavCueParticles(ctx, target.point, color.role, scale, seed, seed2, t)
...
function drawNavCueParticles(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const angle = ps * TAU + Math.sin(t * 0.2 + pa * 6) * 0.25
    ...
    ctx.fillStyle = tokenRgba(colorRole, alpha)
    ctx.beginPath()
    ctx.arc(px, py, (0.6 + 0.3 * envelope) * scale, 0, TAU)
    ctx.fill()
  }
}
...
function drawNavCueRimArc(
  ...
  ctx.lineCap = "round"
  ctx.lineJoin = "round"
  ctx.shadowColor = tokenRgba(color.role, 0.45)
  ctx.shadowBlur = 6

  // 幅広の光層 (低 alpha で柔らかい光)
  ctx.strokeStyle = tokenRgba(color.role, 0.18 * breathe)
  ctx.lineWidth = 3.2
  ctx.beginPath()
  ...
  // 芯線 (明るめ、細い)
  ctx.strokeStyle = tokenRgba(color.role, 0.5 * breathe)
  ctx.lineWidth = 1.3
  ctx.beginPath()
