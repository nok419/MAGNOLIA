...
import { rgbTripletRgba, tokenRgba } from "@/app/visual-tokens"
import { worldToCanvasPoint } from "@/render/shared/coordinates"
import { lerpScalar, seededUnit, TAU } from "./math"
...
export type TrailState = {
  ...
  smoothY: number
  stoppedAt: number
  lastMoteEmissionMs: number
  nodes: TrailNode[]
  lightMotes: LightMote[]
}
...
export function syncTrailPreviousPosition(state: TrailState, wx: number, wy: number) {
  state.prevPlayerX = wx
  state.prevPlayerY = wy
}

export function updateTrail(state: TrailState, wx: number, wy: number, now: number) {
  ...
  if (isMoving) {
    ...
    for (let step = 1; step <= sampleCount; step += 1) {
      ...
      state.smoothX = lerpScalar(state.smoothX, targetX, TRAIL_SMOOTHING)
      state.smoothY = lerpScalar(state.smoothY, targetY, TRAIL_SMOOTHING)
      const sampleTime = now - (sampleCount - step) * 16
      pushTrailNode(state, state.smoothX, state.smoothY, sampleTime)
    }
...
function pushTrailNode(state: TrailState, wx: number, wy: number, time: number) {
...
function resetTrailState(state: TrailState, wx: number, wy: number) {
...
function trimTrailNodeBudget(state: TrailState) {
...
function emitTrailMote(state: TrailState, now: number) {
  if (
...
function trailOpacity(state: TrailState, nodeIndex: number, nodeCount: number, now: number): number {
...
export function drawTrail(
  ctx: CanvasRenderingContext2D, state: TrailState, now: number,
  ...
  for (let i = 0; i < N; i++) {
    const p = worldToCanvasPoint(viewport, W, H, pad, state.nodes[i].wx, state.nodes[i].wy)
    cx[i] = p.x
    cy[i] = p.y
  }
  ...
  for (let pass = 0; pass < 4; pass++) {
    for (let i = 0; i < pathPointCount - 1; i++) {
      ...
      if (pass === 0) {
        ctx.strokeStyle = rgbTripletRgba(color.outer, 0.026 * fade)
        ctx.lineWidth = 1.35 + 5.6 * fade
      } else if (pass === 1) {
        ctx.strokeStyle = rgbTripletRgba(color.mid, 0.14 * fade)
        ctx.lineWidth = 0.55 + 2.2 * fade
      } else if (pass === 2) {
        ctx.strokeStyle = rgbTripletRgba(color.spine, 0.55 * fade)
        ...
        ctx.strokeStyle = tokenRgba("text", 0.62 * fade * intensity)
...
function drawTrailOrnaments(
  ...
  ctx.lineCap = "round"
  ctx.lineJoin = "round"
  ctx.shadowColor = tokenRgba("line", 0.46)
  ctx.shadowBlur = 7
  ...
  for (let i = 3; i < points.length - 4; i += 6) {
    ...
    if (cadence % 3 === 0) {
      ctx.strokeStyle = tokenRgba("text", 0.2 * fade * pulse)
    ...
    if (cadence % 4 === 1) {
      ctx.fillStyle = tokenRgba("text", 0.24 * fade)
      ctx.beginPath()
      ctx.arc(
...
function drawLightMotes(
  ...
  for (let i = state.lightMotes.length - 1; i >= 0; i--) {
    ...
    const sp = worldToCanvasPoint(viewport, W, H, pad, m.wx, m.wy)
