...
import type { ProjectileRendererInput } from "./types"
import { PHI_INV, TAU, alphaColor, clamp01, easeInOutCubic, hashString, normalizeCanvasVector } from "./math"

const BATTLE_PLAYER_PROJECTILE_RENDERERS: Record<
  string,
  (ctx: CanvasRenderingContext2D, input: ProjectileRendererInput) => void
> = {
  playerCarrier: (ctx, input) => drawCarrierProjectile(ctx, input.projectile),
  playerCarrierBlast: (ctx, input) => drawCarrierBlast(ctx, input.projectile, input.timeMs),
  playerMelee: (ctx, input) => drawPulseMelee(ctx, input.projectile, input.timeMs),
  playerPulse: (ctx, input) => drawDefaultPlayerProjectile(ctx, input.projectile),
...
const BATTLE_ENEMY_PROJECTILE_RENDERERS: Record<
  ...
  (ctx: CanvasRenderingContext2D, input: ProjectileRendererInput) => void
> = {
  geoDiamond: (ctx, input) => drawGeoDiamondProjectile(ctx, input.projectile, input.timeMs),
  enemyLance: (ctx, input) => drawEnemyLanceProjectile(ctx, input.projectile, input.timeMs),
  bossCore: (ctx, input) => drawBossCoreProjectile(ctx, input.projectile, input.timeMs),
  signalShard: (ctx, input) => drawSignalShardProjectile(ctx, input.projectile, input.timeMs),
  noiseOrb: (ctx, input) => drawNoiseOrbProjectile(ctx, input.projectile, input.timeMs),
...
export function drawPlayerProjectile(
  ...
  const renderer = resolveBattleRenderer(
    ...
    [p.visual.rendererKind, "default"],
  )
  if (p.visual.auraKind === "inversePhase") {
...
function drawDefaultPlayerProjectile(
  ...
  const radius = p.radius * p.visual.radiusScale * p.visual.scale
  ...
  // Outer aura
  ctx.shadowColor = p.visual.glowColor
  ctx.shadowBlur = 8 * Math.max(0.5, p.visual.glowStrength)
  
  // fading tail
  const tailGrad = ctx.createLinearGradient(x, y - length * 0.3, x, y + length * 1.5)
  tailGrad.addColorStop(0, "rgba(230, 248, 255, 1)")
  tailGrad.addColorStop(0.3, alphaColor(p.visual.accentColor, 0.6))
  tailGrad.addColorStop(1, alphaColor(p.visual.accentColor, 0))
...
function drawInversePhaseProjectileAura(
  ...
  ctx.lineWidth = 0.7
  ctx.strokeStyle = alphaColor(p.visual.secondaryColor ?? p.visual.accentColor, 0.26)
  ctx.shadowColor = alphaColor(p.visual.accentColor, 0.38)
  ctx.shadowBlur = 8
  ctx.beginPath()
...
function drawCarrierProjectile(
  ...
  const radius = p.radius * p.visual.radiusScale * p.visual.scale
  const tailLength = radius * 8.5
  const halfWidth = Math.max(1.6, radius * 0.48)
  ...
  ctx.save()
  ctx.shadowColor = p.visual.glowColor
  ctx.shadowBlur = 14 * Math.max(0.5, p.visual.glowStrength)
...
function drawCarrierBlast(
  ...
  ctx.save()
  ctx.shadowColor = p.visual.glowColor
  ctx.shadowBlur = 16 * Math.max(0.5, p.visual.glowStrength)
  ctx.strokeStyle = alphaColor(p.visual.secondaryColor ?? p.visual.accentColor, 0.82)
  ctx.lineWidth = 1.6
  ctx.beginPath()
  ctx.arc(x, y, radius * pulse, 0, TAU)
  ctx.stroke()

  ctx.strokeStyle = alphaColor(p.visual.accentColor, 0.4)
  ctx.lineWidth = 0.9
  ctx.beginPath()
  ctx.arc(x, y, radius * 0.62 * pulse, 0, TAU)
  ctx.stroke()
  ctx.restore()
}
...
function drawPulseMelee(
  ...
  const sweepSpan = ((p.meleeSweepArcDeg ?? 120) * Math.PI) / 180
  const bladeAngle = angle - sweepSpan * 0.5 + sweepSpan * progress
  const radius = p.radius * p.visual.radiusScale * p.visual.scale
  const bladeLength = radius * 1.28
  const bladeStart = Math.max(12, radius * 0.13)
  const bladeWidth = Math.max(7.4, radius * 0.105)
  const impactPulse = Math.sin(Math.PI * swingPhase)
  const surfacePulse = 0.9 + Math.sin(t * 0.008) * 0.06
  const grip = {
    x: x - Math.cos(bladeAngle) * Math.max(15, radius * 0.22),
    y: y - Math.sin(bladeAngle) * Math.max(15, radius * 0.22),
  }
  ...
  ctx.lineJoin = "round"
  ctx.shadowColor = p.visual.glowColor
  ctx.shadowBlur = 17 * Math.max(0.5, p.visual.glowStrength)
  ctx.globalCompositeOperation = "lighter"
  ...
  ctx.quadraticCurveTo(
    x + Math.cos(bladeAngle) * radius * 0.42,
    y + Math.sin(bladeAngle) * radius * 0.42,
    blade.tip.x,
    blade.tip.y,
  )
  ctx.stroke()
  ...
  ctx.beginPath()
  ctx.arc(blade.tip.x, blade.tip.y, Math.max(3.4, radius * 0.052), 0, TAU)
  ctx.fill()
  ...
  ctx.beginPath()
  ctx.moveTo(
    x + Math.cos(angle - sweepSpan * 0.5) * radius * 0.28,
    y + Math.sin(angle - sweepSpan * 0.5) * radius * 0.28,
  )
  ctx.lineTo(
    x + Math.cos(angle - sweepSpan * 0.5) * radius * 0.48,
    y + Math.sin(angle - sweepSpan * 0.5) * radius * 0.48,
  )
  ctx.moveTo(
    x + Math.cos(angle + sweepSpan * 0.5) * radius * 0.28,
    y + Math.sin(angle + sweepSpan * 0.5) * radius * 0.28,
...
function drawNoiseOrbProjectile(
  ...
  const { x, y } = p.position
  const r = p.radius * p.visual.radiusScale * p.visual.scale
  const seed = hashString(p.projectileInstanceId) + p.visual.seedBucket
