...
const ENEMY_VISUAL_PROFILES: Record<string, CircleEnemyProfile> = {
  a1: {
    accent: tokenRgba("memory", 0.86),
    glow: tokenRgba("memory", 0.7),
  ...
  heavy: {
    accent: tokenRgba("danger", 0.78),
    glow: tokenRgba("danger", 0.78),
...
const ENEMY_BOSS_VISUAL_PROFILES: Record<string, CircleBossProfile> = {
  c1: {
    accent: tokenRgba("memory", 0.86),
    secondary: tokenRgba("text", 0.88),
    glow: tokenRgba("memory", 0.86),
  ...
  b1: {
    accent: tokenRgba("danger", 0.86),
    secondary: tokenRgba("memory", 0.88),
    glow: tokenRgba("danger", 0.94),
    outerOrbitScale: 2.7,
    innerOrbitScale: 1.7,
    midOrbitScale: 2.18,
    iconSize: 4,
...
function drawCircleEnemy(
  ...
  const baseColor = enemy.burning ? tokenRgba("danger", 0.82) : profile.accent
  ...
  if (hpRatio < 1) {
    ...
    ctx.strokeStyle = tokenRgba("danger", 1)
    ctx.lineWidth = 1
    ctx.beginPath()
