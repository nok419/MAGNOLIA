...
import { TAU, alphaColor, easeOutCubic, hashString, seededRandom } from "./math"
...
export function drawHazard(
  ...
  hazard: {
    ...
    visual: BattleRenderState["hazards"][number]["visual"]
  ...
  if (isTelegraph) {
    /* ── 予告フェーズ: ダッシュ枠 + グリッチ予兆 ── */
    const flashHz = Math.max(0.1, hazard.visual.telegraphFlashHz)
    const pulse = reduceFlashing
      ? 0.54 + 0.1 * Math.sin(t * 0.006)
      : 0.45 + 0.35 * Math.sin(t * 0.001 * flashHz * TAU)

    ctx.fillStyle = alphaColor(hazard.visual.telegraphColor, hazard.visual.telegraphOpacity * (0.35 + pulse * 0.35))
    ctx.fillRect(x, y, w, h)

    // 動くダッシュ枠
    ctx.strokeStyle = alphaColor(hazard.visual.telegraphColor, 0.32 + pulse * 0.38)
    ctx.lineWidth = 2
    ctx.setLineDash([8, 8])
    ...
    for (let i = 0; i < 3; i++) {
      if (seededRandom(gf + i * 37) > 0.6) {
        ...
        ctx.fillStyle = alphaColor(hazard.visual.telegraphColor, 0.8)
    ...
    const fogAlpha = phaseAlpha * Math.max(0.08, hazard.visual.activeOpacity * 0.58)
    const fogFeather = Math.max(12, hazard.visual.edgeFeather)
    ...
    // 暗い下地 (既存 — コア内のみ)
    ctx.globalAlpha = 1
    ctx.fillStyle = alphaColor(hazard.visual.activeColor, 0.08 + phaseAlpha * 0.14)
    ctx.fillRect(x, y, w, h)
