...
現状の playable slice は、探索・戦闘・アーカイブ・装備の基本構造は成立している。一方で、体験としては「通信を受信しているゲーム」よりも「通信テキストが横に出ている縦STG」に見えやすい。
...
- 現状は 85 秒程度の本文で、3 人の声が順番に流れる。やや長い。
- 45〜65 秒程度へ圧縮するか、3 人の短い通信束としてテンポを上げる。
...
重要: 現在の `tx_evacuation.chunks.json` は本文が 0〜8500ms までしかない一方、`mission_evacuation.json` は `durationMs: 60000` になっている。音声未実装段階では、長い無字幕時間が生まれ、体験の空白になりやすい。
...
この方向で、MAGNOLIA は普通の STG ではなく、「言葉を欠損させないためのSTG」「失われた通信を探す探索ゲーム」として立ち上がる。
